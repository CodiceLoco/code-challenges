...
class Ride(object):
    def __init__(self, startR, startC, finishR, finishC, earliestStart, latestFinish):  # noqa
        self.startR = startR
        self.startC = startC
    ...
    def takeAction(self):
        ...
        if self.moving <= 0:
            if self.rewardToTake:
                table.addScore(self.actions[self.actualAction], self.startRide, self.eventualReward)  # noqa
                self.rewardToTake = False
                self.eventualReward = 0
            ...
            if self.actualAction < len(self.actions):
                ...
                if (self.row == ride.startR) & (self.col == ride.startC):
                    if table.time >= ride.earliestStart:
                        self.moving = self.estimateTime(ride.finishR, ride.finishC)  # noqa
                        self.eventualReward = self.estimateTime(ride.finishR, ride.finishC)  # noqa
    ...
    def score(self, inputF, submissionF):
        with open(inputF) as f:
            ...
            for x in range(N):
                line = line + 1
                nLine = content[line].split(" ")
                rid = Ride(int(nLine[0]), int(nLine[1]), int(nLine[2]), int(nLine[3]), int(nLine[4]), int(nLine[5]))  # noqa
                Rides.append(rid)
...
if __name__ == '__main__':
    ...
    j = JudgeSystem(file_name, output_filename)
    print(f"Score of {file_name}: {j.score}")
